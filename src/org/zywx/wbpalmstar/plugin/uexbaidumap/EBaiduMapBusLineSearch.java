...
public class EBaiduMapBusLineSearch implements OnGetPoiSearchResultListener,
	...
	public void nextBusLineNode() {
		if (busNodeIndex < (mBusRoute.getStations().size() - 1)) {
			// 索引加
			busNodeIndex++;
			showBusNode();
		}
	...
	private void showBusNode() {
		...
		popupText.setBackgroundResource(EUExUtil
				.getResDrawableID("plugin_map_bubble_bg_default"));
		popupText.setTextColor(0xff000000);
		// 移动到指定索引的坐标
		mBaiduMap.setMapStatus(MapStatusUpdateFactory.newLatLng(mBusRoute
				.getStations().get(busNodeIndex).getLocation()));
		// 弹出泡泡
		popupText.setText(mBusRoute.getStations().get(busNodeIndex).getTitle());
		mBaiduMap.showInfoWindow(new InfoWindow(popupText, mBusRoute
				.getStations().get(busNodeIndex).getLocation(), 0));
	}
